...
int[,] readPoints()
{
    ...
    Console.Write("Ведите координаты точек А и В в формате A(3,6); B(2,1): ");
    ...
    if (inputLine != null)
    {
        try 
        {
            ...
            string pattern1 = @"A\((-?\d+),(-?\d+)\);";
            string pattern2 = @"B\((-?\d+),(-?\d+)\)";
...
void printDistance(int[,] array)
{
    try 
    {
        ...
        double distance = Math.Sqrt(Math.Pow(array[1, 0] - array[0, 0], 2) + Math.Pow(array[1, 1] - array[0, 1], 2));
